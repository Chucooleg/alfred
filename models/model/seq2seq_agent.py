...
class Module(nn.Module):
    ...
    def run_train(self, splits, args=None, optimizer=None, start_epoch=0, end_epoch=20, start_iters=None, log_memory=True):
        ...
        # splits
        train = splits['train']
        augmentation = splits['augmentation'] if self.args.use_augmentation else []
        ...
        if self.args.dataset_fraction > 0:
            ...
            augmentation = augmentation[:small_train_size] 
        ...
        for epoch in trange(start_epoch, end_epoch, desc='epoch'):
            ...
            # write stats to file as well
            fstats = os.path.join(args.dout, 'stats_epoch_%d.json' % epoch)
            with open(fstats, 'wt') as f:
                json.dump(stats, f, indent=2)           
    ...
    def make_debug(self, preds, data):
        ...
        for task in data:
            ...
            debug[i] = {
                'lang_goal': ex[ann_key]['anns'][ex['ann']['repeat_idx']]['task_desc'],
    ...
    def get_task_root(self, ex):
        ...
        return os.path.join(data_dir, split_dir, *(ex['root'].split('/')[-2:]))

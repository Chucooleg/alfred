...
def load_task_json(args, model, task, rewrite_new_traj=False):
    '''
    load preprocessed demo json from disk
    '''
    # HACK
    if rewrite_new_traj:
        json_path = os.path.join(args.dout, task['task'], 'traj_data.json')
    else:
...
def overwrite_task_json(args, model, task, ex):
    '''
    ...
    task_dir = os.path.join(args.dout, task['task'])
    json_path = os.path.join(task_dir, 'traj_data.json')
...
def make_demo_output(args, preds, data, model, level='low'):
    '''
    make language output for demo
    '''
    outputs = {}
    for task in data:
        ex = load_task_json(args, model, task)
...
def write_ann_to_traj(args, preds, data, model, level='low'):
    '''write predicted language back to trajectory on disk'''

    assert level in ['low', 'high']

    for task in data:
        ex = load_task_json(args, model, task, rewrite_new_traj=True if level == 'low' else False)
        ...
        overwrite_task_json(args, model, task, ex)


def pred_and_save(args, split, split_name, model, batch_size, dout, level='low', debug=False):
    assert level in ['low', 'high']
    ...
    # make language output for demo
    split_outputs = make_demo_output(args, split_preds, split, model, level)
    ...
    # write predicted language back to trajectory on disk
    write_ann_to_traj(args, split_preds, split, model, level)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dout', help='dataset folder', default='/root/data_alfred/demo_generated/')
    parser.add_argument('--splits', help='json file containing train/dev/test splits. e.g. /root/data_alfred/splits/demo_T***.json')
    ...
    parser.add_argument('--preprocess', action='store_true', help='preprocess actions and object states using vocab')
    ...
    TIME_STAMP = re.findall('.*T(.*).json', args.splits)[0]
    ...
    # args.dout = os.path.join(args.dout, f'new_trajectories_T{TIME_STAMP}', f'dout_explainer_low_{low_mod_name}_high_{high_mod_name}')
    # data_alfred/demo_generated/new_trajectories_T.../
    args.dout = os.path.join(args.dout, f'new_trajectories_T{TIME_STAMP}')

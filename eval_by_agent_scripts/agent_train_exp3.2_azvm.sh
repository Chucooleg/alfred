...
export SPLITS=$ALFRED_ROOT/data/splits/data_augmentation_experiment3_20200903.json
echo Split file $SPLITS

# echo '-------------------------------'
# # 3.1 Half of original training set, plus the other hald annotated by Explainer
# export AUTOGENERATION_LANG_MODEL=explainer
# # training and eval script
# bash $SCRIPT_HOME/exp_3.1_train_azvm2.sh

echo '-------------------------------'
# 3.2 Half of original training set, plus the other hald annotated by Baseline
export AUTOGENERATION_LANG_MODEL=baseline
# training and eval script
bash $SCRIPT_HOME/exp_3.2_train_azvm.sh
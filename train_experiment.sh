...
export DOUT=$PT_OUTPUT_DIR/exp_philly/model:$MODEL,name:v0_epoch_${EPOCH}_${SUFFIX}
mkdir $DOUT
...
command="python -u models/train/train_seq2seq.py --model $MODEL --dout $DOUT --data $PT_DATA_DIR/$DATA --splits $PT_DATA_DIR/splits/$SPLITS --gpu --epoch $EPOCH $EXTRAS"

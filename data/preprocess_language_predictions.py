...
class Dataset(object):
    ...
    def preprocess_splits(self, splits):
        ...
        for k, d in splits.items():
            ...
            if 'train' in k:
                ...
                raise ValueError('split must be in train, valid_seen or valid_unseen')
            ...
            for task in progressbar.progressbar(d):
                ...
                preprocessed_json_paths.append(preprocessed_json_path)
        ...
        return preprocessed_json_paths
    ...
    def process_language(self, ex, traj, r_idx):
        ...
        # numericalize language
        traj['num'] = {}
        traj['num']['lang_goal'] = self.numericalize(self.vocab['word'], traj['ann']['goal'], train=False)
        traj['num']['lang_instr'] = [self.numericalize(self.vocab['word'], x, train=False) for x in traj['ann']['instr']]
    ...
    def process_actions(self, ex, traj):
        ...
        for a in (ex['plan']['low_actions'] + [end_action]):
            ...
            traj['num']['action_low'][high_idx].append({
                'high_idx': a['high_idx'],
                'action': self.vocab['action_low'].word2index(a['discrete_action']['action'], train=False),
        ...
        for a in ex['plan']['high_pddl']:
            traj['num']['action_high'].append({
                'high_idx': a['high_idx'],
                'action': self.vocab['action_high'].word2index(a['discrete_action']['action'], train=False),
